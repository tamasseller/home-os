...
class Network<S, Args...>::IcmpReplyJob: IpTxJob<IcmpReplyJob>
{
	...
	inline bool onPreparationDone(Launcher *launcher, IoJob* item, Result result)
	{
		Os::assert(result == Result::Done, NetErrorStrings::unknown);
		auto self = static_cast<IcmpReplyJob*>(item);
		...
		if(typeCode == 0x800) {
			...
			self->packet.stage2.write16net(0); // type and code
			self->packet.stage2.write16net(checksum); // type and code

			do {
				Chunk chunk = reader.getChunk();

				bool ok = self->packet.stage2.copyIn(chunk.start, static_cast<uint16_t>(chunk.length()));
		...
		self->request.template dispose<Pool::Quota::Rx>();
    ...
    static bool startReplySequence(Launcher *launcher, IoJob* item) {
		...
		return IcmpReplyJob::IpTxJob::startPreparation(launcher, item, requesterIp, totalLength - headerLength, 0);
    }
